...
const CourseCard = ({
  ...
  includes
    ...
    <div className="space-y-8">
      ...
      {instructors && instructors.length > 0 && <div className="space-y-4">
          ...
          <ul className="list-none space-y-3">
            {instructors.map((instructor, index) => <li key={index} className="text-gray-400 flex items-start gap-2 leading-relaxed text-base">
      ...
      {curriculum && curriculum.length > 0 && <div className="space-y-4">
          <h4 className="text-xl font-montserrat font-semibold flex items-center gap-2 text-gray-300">
            <Award className="text-detective-red" />
            Conteúdo Programático:
          </h4>
          <ul className="grid grid-cols-1 gap-3">
            {curriculum.map((item, index) => <li key={index} className="text-gray-400 flex items-start gap-2 leading-relaxed text-base">
                <CheckCircle2 className="w-5 h-5 text-detective-red shrink-0 mt-1" />
                <span>{item}</span>
              </li>)}
          </ul>
      ...
      <div className="space-y-4">
        ...
        <ul className="list-none space-y-3">
          {includes.map((item, index) => <li key={index} className="text-gray-400 flex items-start gap-2 leading-relaxed text-base">
              <CheckCircle2 className="w-5 h-5 text-detective-red shrink-0 mt-1" />
              <span>{item}</span>
            </li>)}
        </ul>
      ...
      <div className="pt-8 space-y-4 border-t border-gray-800">
        <div className="space-y-3">
          ...
          {priceWithBadge && <Button variant="default" className="w-full group bg-detective-red hover:bg-detective-red/90 text-white transition-all duration-300 font-montserrat text-lg py-6">
...
const Cursos = () => {
  const courses: CourseInfo[] = [{
    ...
    icon: <Brain size={32} />,
    curriculum: ["Fundamentos de inteligência", "Criminalística e ciências forenses", "Inteligência criminal", "Análise de dados forenses", "Contrainteligência", "Técnicas de investigação e monitoramento", "Aspectos legales e éticos da atividade de inteligência"],
    price: "R$ 439,99",
    priceWithBadge: "R$ 639,99",
    includes: ["Diploma", "Funcional"]
  }, {
  ...
  return <div className="min-h-screen bg-detective-black">
      <Header />
